//! Invitation-only access use-cases: the operator toggle, the public waitlist,
//! and the review queue (approve mints a one-time token and emails the accept
//! link; reject is silent), plus token validation and single-use consumption.
//! Owns only the invite-facing ports it needs, so a consumer that just runs the
//! waitlist doesn't depend on the whole app surface.
...
use crate::{
    AcceptInviteError, ApproveInviteError, Clock, InviteRequestStore, Notifier, RequestInviteError,
    Result, SettingsStore, UserStore,
};

/// Invitation-only access use-cases, over just the invite-facing ports.
#[derive(Clone)]
pub struct InviteService {
    invites: Arc<dyn InviteRequestStore>,
