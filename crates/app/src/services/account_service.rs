//! Account use-cases: registration (real, dev-puppet, and barred), handle/email
//! lookup, credential and handle authentication, key enrolment, feed-paging
//! preference, age verification, and the two cross-cut guards
//! ([`verify_user_action`](AccountService::verify_user_action),
//! [`ensure_not_barred`](AccountService::ensure_not_barred)). Owns only the
//! account-facing ports it needs, so a consumer that just registers or
//! authenticates doesn't depend on the whole app surface.
...
use crate::{
    AgeVerifier, AuthenticateError, Clock, EnrollPublicKeyError, EnsureBarredAccountError,
    FoundDemosError, RegisterAccountError, Result, SetFeedPagingError, StoreError, UserStore,
    VerifyActionError,
};

/// Account use-cases, over just the account-facing ports.
#[derive(Clone)]
pub struct AccountService {
    users: Arc<dyn UserStore>,
    age_verifier: Arc<dyn AgeVerifier>,
...
impl AccountService {
    ...
    /// `cast_vote`). Enrolment is **first-key-only** here: a key
    ...
    pub async fn ensure_not_barred(&self, user: UserId) -> Result<(), FoundDemosError> {
        let u = self.users.get(user).await?.ok_or(StoreError::NotFound)?;
