//! Governance use-cases: opening proposals, casting signed ballots, closing and
//! tallying a proposal, and applying the demos-level effects of one that passed.
//! Owns the governance-facing ports and leans on three peers — the account
//! service to verify a signed ballot, the moderation service for the electorate
//! weight sum, and the membership service for a demos's phase.

use std::sync::Arc;

use domain::{
    DemosId, Phase, Proposal, ProposalId, ProposalKind, ProposalStatus, Rule, UserId, VoteWeighting,
    MAX_SANCTION_DAYS,
};

use crate::identity::vote_message::vote_message;
use crate::{
    CastVoteError, Clock, CloseProposalError, DemosStore, MembershipStore, OpenProposalError,
    ProposalStore, Result, RuleStore, StoreError, VoteStore,
};
...
/// Governance use-cases, over just the governance-facing ports plus the account,
/// moderation, and membership peers.
#[derive(Clone)]
...
impl GovernanceService {
    ...
    pub async fn open_proposal(
        ...
        if kind.decision_class() == domain::DecisionClass::Constitutional
            && self.membership.phase_of(demos).await? == Phase::Seed
        {
