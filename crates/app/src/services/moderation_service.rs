//! Moderation use-cases: reporting/flag-merging, empanelling and settling trials,
//! trial-gallery comments and their notifications, jury voting, ban-term
//! derivation, the electorate weight sum, and the posting/sanction guards. Owns
//! the moderation-facing ports and calls [`AccountService`](super::account_service::AccountService)
//! (its one peer) to verify a juror's signed ballot.

use std::sync::Arc;

use domain::{
    reach_verdict, select_jury, ContentScale, DemosId, Membership, NotificationKind,
    PostingPolicy, Report, ReportId, ReportReason, ReportStatus, ReportTarget, RuleId, Timestamp,
    Trial, TrialComment, TrialId, UserId, Verdict, VoteWeighting, MAX_SANCTION_DAYS,
};

use crate::identity::jury_vote_message::jury_vote_message;
use crate::{
    CanPostError, CastJuryVoteError, Clock, CommentOnTrialError, CommentStore, DemosStore,
    MemberActionError, MembershipStore, NotificationStore, OpenTrialError, PostStore, ReportStore,
    Result, RuleStore, SettleTrialError, StoreError, TrialCommentStore, TrialStore, UserStore,
};

use super::account_service::AccountService;
use super::posting_allowed::posting_allowed;

/// Moderation use-cases, over just the moderation-facing ports plus the account peer.
#[derive(Clone)]
pub struct ModerationService {
    reports: Arc<dyn ReportStore>,
    trials: Arc<dyn TrialStore>,
    trial_comments: Arc<dyn TrialCommentStore>,
...
impl ModerationService {
    ...
    pub async fn file_or_merge_flag(
        &self,
        demos: DemosId,
    ...
    /// mirroring the mention notifier: called after the comment is stored.
    ...
    /// counts.
    pub async fn total_voter_weight(
        &self,
        demos: DemosId,
    ...
    pub async fn require_unsanctioned_member(
        &self,
        user: UserId,
        demos: DemosId,
    ...
    /// Backs both the enforcement in `create_post` and the composer's community
    /// picker (so it only offers postable communities).
    ...
    pub async fn require_can_post(&self, user: UserId, demos: DemosId) -> Result<(), CanPostError> {
        let d = self.demoi.get(demos).await?.ok_or(StoreError::NotFound)?;
        let m = self.memberships.get(user, demos).await?;
