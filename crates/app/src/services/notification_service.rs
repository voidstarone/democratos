//! In-app notification use-cases: reading a member's notifications, the toolbar
//! badge count, marking them seen, per-kind alert preferences, and fanning out
//! `@handle` mention alerts. Owns only the notification + user ports it needs, so
//! a consumer that just shows the bell doesn't depend on the whole app surface.

use std::sync::Arc;

use domain::{mentions, CommentId, Notification, NotificationKind, PostId, UserId};

use crate::{Clock, NotificationStore, Result, UserStore};
