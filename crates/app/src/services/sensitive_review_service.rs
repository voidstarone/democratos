//! Platform-wide sensitive-content review use-cases: the reviewer opt-in, human
//! flagging (hides pending review + opens a case), the reviewer queue and badge
//! count, quorum voting with disposition, and the age-gated visibility decision.
//! Owns only the review-facing ports it needs, so a consumer that just runs the
//! review queue doesn't depend on the whole app surface.

use std::sync::Arc;

use domain::{
    outcome_for, Post, ReportTarget, ReviewOutcome, SensitiveCase, SensitiveCaseId, SensitiveTag,
    UserId,
};
use domain::{visibility, Visibility};

use crate::{
    Clock, CommentStore, PostStore, Result, SensitiveCaseStore, SensitiveReviewError, StoreError,
    UserStore,
};

use super::escalate_to_operator::escalate_to_operator;

/// Sensitive-content review use-cases, over just the review-facing ports.
#[derive(Clone)]
pub struct SensitiveReviewService {
    sensitive_cases: Arc<dyn SensitiveCaseStore>,
